...
{
    public static class ThingFilterUI_Nutrition
    {
        public static void DoThingFilterConfigWindow(Rect rect, ThingFilterUI_Nutrition.UIState state, ThingFilter filter, List<CompRefuelableStat> _cachedFuelBuildings, ThingFilter parentFilter = null, int openMask = 1, IEnumerable<ThingDef> forceHiddenDefs = null, IEnumerable<SpecialThingFilterDef> forceHiddenFilters = null, bool forceHideHitPointsConfig = false, bool forceHideQualityConfig = false, bool showMentalBreakChanceRange = false, List<ThingDef> suppressSmallVolumeTags = null, Map map = null)
        {
            ...
            {
                ThingFilterUI_Nutrition.DrawHitPointsFilterConfig(ref num2, viewRect.width, filter);
            }
            if (flag2 && !forceHideQualityConfig)
            {
                ThingFilterUI_Nutrition.DrawQualityFilterConfig(ref num2, viewRect.width, filter);
            }
            if (ModsConfig.AnomalyActive && showMentalBreakChanceRange)
            {
                ThingFilterUI_Nutrition.DrawMentalBreakFilterConfig(ref num2, viewRect.width, filter);
            }
            float num3 = num2;
            Rect rect4 = new Rect(0f, num2, viewRect.width, 9999f);
            visibleRect.position -= rect4.position;
            Listing_TreeThingFilter_Nutrition listing_TreeThingFilter = new (filter, parentFilter, forceHiddenDefs, forceHiddenFilters, suppressSmallVolumeTags, state.quickSearch.filter, _cachedFuelBuildings);
