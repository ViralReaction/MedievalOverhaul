...
namespace MedievalOverhaul
{
    public class JobDriver_DoMending : JobDriver
    {
        ...
        public IBillGiver BillGiver
        {
            get
            {
                if (MendingBenchTarget is not IBillGiver billGiver)
                {
        ...
        protected Thing MendingBenchTarget
        {
            get
            {
        ...
        public override IEnumerable<Toil> MakeNewToils()
        {
            ...
            this.FailOn(delegate ()
            {
                ...
                {
                    if (this.job.bill.DeletedOrDereferenced || !billGiver.CurrentlyUsableForBills())
                    {
                        return true;
                    }
                }
            ...
            {
                if (this.job.targetQueueB?.Count == 1 && this.job.targetQueueB[0].Thing is UnfinishedThing unfinishedThing)
                {
      ...
      bool placeInBillGiver = false)
        ...
        public static Toil DoRecipeWork_Mend()
        {
            ...
            toil.tickAction = delegate ()
            {
                ...
                if (toil.actor.CurJob.GetTarget(TargetIndex.A).Thing is not null && toil.actor.CurJob.GetTarget(TargetIndex.A).Thing is IBillGiverWithTickAction billGiverWithTickAction)
                {
                ...
                {
                    if (curJob.RecipeDef.workTableSpeedStat is not null && jobDriver_DoBill.BillGiver is Building_WorkTable building_WorkTable)
                    {
        ...
        {
            if (job.GetTarget(TargetIndex.B).Thing is not null && job.GetTarget(TargetIndex.B).Thing is UnfinishedThing unfinishedThing)
            {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            if (job.RecipeDef.autoStripCorpses)
                            {
                                ...
                                if (thing is not null && thing is IStrippable && strippable.AnythingToStrip())
                                {
