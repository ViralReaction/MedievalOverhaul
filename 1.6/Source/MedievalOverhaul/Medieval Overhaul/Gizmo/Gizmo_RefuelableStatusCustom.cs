...
namespace MedievalOverhaul
{
    ...
    public class Gizmo_RefuelableStatusCustom : Gizmo
    {
        public Gizmo_RefuelableStatusCustom()
        {
            this.Order = -100f;
        }
        ...
        public override GizmoResult GizmoOnGUI(Vector2 topLeft, float maxWidth, GizmoRenderParms parms)
        {
            ...
            Find.WindowStack.ImmediateWindow(1523289473, overRect, WindowLayer.GameUI, delegate
            {
                ...
                Widgets.FillableBar(rect3, fillPercent, Gizmo_RefuelableStatusCustom.FullBarTex, Gizmo_RefuelableStatusCustom.EmptyBarTex, false);
                if (this.refuelable.Props.targetFuelLevelConfigurable)
                {
                    ...
                    float x = rect3.x + num * rect3.width - (float)Gizmo_RefuelableStatusCustom.TargetLevelArrow.width * 0.5f / 2f;
                    float y = rect3.y - (float)Gizmo_RefuelableStatusCustom.TargetLevelArrow.height * 0.5f;
                    GUI.DrawTexture(new Rect(x, y, (float)Gizmo_RefuelableStatusCustom.TargetLevelArrow.width * 0.5f, (float)Gizmo_RefuelableStatusCustom.TargetLevelArrow.height * 0.5f), Gizmo_RefuelableStatusCustom.TargetLevelArrow);
                }
